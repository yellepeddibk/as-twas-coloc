% poster.tex -- Undergraduate Research
...
\usepackage[orientation=landscape, size=custom, width=121.92, height=91.44, scale=1.08]{beamerposter}
...
\usepackage{qrcode}
\usepackage{caption}
\captionsetup{font=small, labelfont=bf, labelsep=period, skip=0.25em}
...
% ══════════════════════════════════════════════════════════════════════════════
% COLUMN 1 (left): Introduction, Key Question, Methods, Discussion
% ══════════════════════════════════════════════════════════════════════════════
...
\vskip0.3em
Because many AS-associated variants do not directly implicate a protein-coding
gene, linking disease risk to \textit{tissue-specific gene regulation} is an
important step toward identifying biologically meaningful mechanisms and
therapeutic targets.
...
signal share the same causal variant, helping distinguish high-confidence
regulatory mechanisms from LD-driven associations.
\end{block}
...
\begin{tikzpicture}[
  ...
  data/.style={rectangle, rounded corners=3pt, draw=ISUGold, fill=ISUGold!15,
               text width=9em, minimum height=1.8em, align=center,
               font=\footnotesize},
  arrow/.style={-{Stealth[length=2.5mm]}, thick, ISUDark}
]
  \node[data] (gwas) {AS GWAS (GCST005529)\\$N$=25,764};
  \node[data, right=1.6cm of gwas] (gtex) {GTEx v8 Mashr Models\\8 tissues};
  \node[box, below=2cm of $(gwas)!0.5!(gtex)$] (harm) {1. GWAS QC \& Harmonization};
  ...
  \coordinate (junc) at ($(harm.north)+(0,0.5)$);
  \draw[thick, ISUDark] (gwas.south) |- (junc);
  \draw[thick, ISUDark] (gtex.south) |- (junc);
  \draw[arrow] (junc) -- (harm.north);
...
\begin{itemize}\setlength{\itemsep}{2pt}
  \item \textit{ERAP1} (MHC-I antigen processing) is a known AS gene,
    validating our pipeline, and colocalizes in Whole Blood and Small Intestine.
  ...
  \item \textit{UBE2L3} (ubiquitin-proteasome pathway) may interact with
    \textit{ERAP1} in antigen processing, suggesting a shared mechanistic axis.
  \item Whole Blood was the most frequently enriched tissue (11 of 35 pairs),
    consistent with AS being a systemic immune-mediated disease.
\end{itemize}
\end{block}

% ── Predicted Protein Interaction Network ───────────────────────────────────
\begin{block}{Predicted Protein Interaction Network}
\begin{center}
\hspace*{-10cm}\includegraphics[width=1.5\linewidth]{figures/string_network.png}
\captionof{figure}{STRING-DB v12 subgraph of 16 prioritized proteins
(score $\geq 0.400$). Edges: \textit{CARD9}--\textit{ERAP1},
\textit{ZNF646}--\textit{ZNF668}, \textit{PRSS53}--\textit{ZNF646}/\textit{ZNF668}.}
\end{center}
\end{block}
...
% ══════════════════════════════════════════════════════════════════════════════
% COLUMN 2 (center): TWAS Results, Coloc Results, Conclusion, Future
% ══════════════════════════════════════════════════════════════════════════════
...
\justifying
S-PrediXcan tested \textbf{2,313 gene-tissue pairs} across 8 GTEx v8
...
\vskip0.3em
\begin{center}
\includegraphics[width=0.84\linewidth]{figures/manhattan_twas.png}
\captionof{figure}{Manhattan-style plot of TWAS $-\!\log_{10}(p)$ across 8
tissues. Top gene symbols annotated. Dashed line: FDR $< 0.05$.}
\end{center}
\begin{center}
\includegraphics[width=0.46\linewidth]{figures/qqplot_twas.png}
...
\vskip0.3em
\begin{center}
\includegraphics[width=0.54\linewidth]{figures/coloc_pp4_vs_pp3pp4.png}
...
\vskip0.3em
This framework improves interpretability by narrowing a broad set of TWAS
signals to a smaller group of higher-confidence gene-tissue pairs, providing
a focused set of candidates for downstream fine-mapping and functional
validation in AS.
\end{block}
...
\begin{itemize}\setlength{\itemsep}{2pt}
  \item Residue-level binding-interface dissection of top STRING-predicted interactions via Seq2Bind [1]
  \item Fine-mapping (SuSiE) and validation in independent AS cohorts
...
\begin{center}
\hspace*{-6.5cm}\includegraphics[width=0.78\linewidth,keepaspectratio]{figures/coloc_pp4_barplot.png}
...
\begin{enumerate}\setlength{\itemsep}{0pt}\setlength{\parsep}{0pt}\setlength{\topsep}{0pt}\setlength{\leftskip}{-1.2em}
  \item Ma X, Dey S, et al. Seq2Bind webserver. \textit{NAR Genom Bioinform}. 2025;7:lqaf154.
  \item Ellinghaus D, et al. Multi-disease inflammatory GWAS meta-analysis. \textit{Nat Genet}. 2016;48:510--518.
  \item GTEx Consortium. Regulatory effects across human tissues (GTEx atlas). \textit{Science}. 2020;369:1318--1330.
  \item Barbeira AN, et al. S-PrediXcan. \textit{Nat Commun}. 2018;9:1825.
  \item Giambartolomei C, et al. coloc.abf. \textit{PLoS Genet}. 2014;10:e1004383.
  \item Szklarczyk D, et al. STRING v12. \textit{Nucleic Acids Res}. 2023;51:D685--D699.
\end{enumerate}
\end{block}
...
\justifying
We thank \textbf{Dr.\ Ratul Chowdhury} for mentorship and project guidance.
AS GWAS summary statistics were obtained from the NHGRI-EBI GWAS Catalog
(GCST005529; Ellinghaus et al.\ 2016), with expression models from GTEx v8.
Analyses used S-PrediXcan, coloc.abf, and STRING-DB v12 in Python and R.

\vskip0.2em
\begin{center}
\qrcode[height=4.5cm]{https://github.com/yellepeddibk/as-twas-coloc}\\[0.7em]
\texttt{github.com/yellepeddibk/as-twas-coloc}
\end{center}
\end{block}
